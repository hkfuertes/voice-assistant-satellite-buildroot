...
class MicroWakeWord(TfLiteWakeWord):
    ...
    See: [https://github.com/kahrendt/microWakeWord/](https://github.com/kahrendt/microWakeWord/)
    """
    ...
    def process_streaming(self, features: np.ndarray) -> Optional[bool]:
        ...
        concatenated = np.concatenate(self._features, axis=1)
        scaled_features = concatenated / self.input_scale + self.input_zero_point
        rounded_features = np.round(scaled_features)
        # Clampea a rango válido para uint8
        clamped_features = np.clip(rounded_features, 0, 255)
        quant_features = clamped_features.astype(np.uint8)
